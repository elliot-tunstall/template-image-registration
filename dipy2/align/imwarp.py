...
class SymmetricDiffeomorphicRegistration(DiffeomorphicRegistration):
    ...
    def _init_optimizer(self, static, moving, static_mask, moving_mask,
        ...
        self.moving_mask_ss = ScaleSpace(moving_mask, self.levels, moving_grid2world,
                                    moving_spacing, self.ss_sigma_factor,
                                    self.mask0)
        ...
        self.static_mask_ss = ScaleSpace(static_mask, self.levels, static_grid2world,
                                static_spacing, self.ss_sigma_factor,
                                self.mask0)
        ...
        # section was to test if we could run the image maks at a single scale space.
        # see _iterate() below for further implemenation changes.

        # # Acquire current resolution information from scale spaces
        # self.moving_mask = self.moving_mask_ss.get_image(1)
        # self.static_mask = self.static_mask_ss.get_image(1)

        # self.mask_disp_shape = \
        #     self.static_mask_ss.get_domain_shape(1)
        # self.mask_disp_grid2world = \
        #     self.static_mask_ss.get_affine(1)
        # self.mask_disp_world2grid = \
        #     self.static_mask_ss.get_affine_inv(1)
        # self.mask_disp_spacing = \
        #     self.static_mask_ss.get_spacing(1)
    ...
    def _end_optimizer(self):
        ...
        del self.moving_ss
        del self.static_ss
        del self.moving_mask_ss
        del self.static_mask_ss
    ...
    def _iterate(self):
        ...
        # This implementation doesn't support single-scale masks
        moving_mask = self.moving_mask_ss.get_image(self.current_level)
        static_mask = self.static_mask_ss.get_image(self.current_level)
        ...
        # Further changes here where we use the same shape and grid2world for the mask.
        wstatic = self.static_to_ref.transform_inverse(current_static,
                                                       'linear',
                                                       None,
                                                       current_disp_shape,
                                                       current_disp_grid2world)
        wstatic_mask = self.static_to_ref.transform_inverse(static_mask,
                                                       'nearest',                   # could change to 'nearest',
                                                       None,
                                                       current_disp_shape,
                                                       current_disp_grid2world)
        wmoving = self.moving_to_ref.transform_inverse(current_moving,
                                                       ...
                                                       current_disp_shape,
                                                       current_disp_grid2world)
        wmoving_mask = self.moving_to_ref.transform_inverse(moving_mask,
                                                       'nearest',                   # could change to 'nearest',
                                                       None,
                                                       current_disp_shape,
                                                       current_disp_grid2world)
        ...
        self.metric.set_moving_mask(wmoving_mask, current_disp_grid2world,
                                current_disp_spacing,
                                self.static_direction)
        ...
        self.metric.set_static_image(wstatic, current_disp_grid2world,
                                     current_disp_spacing,
                                     self.static_direction)
        self.metric.set_static_mask(wstatic_mask, current_disp_grid2world,
                                     current_disp_spacing,
                                     self.static_direction)
        ...
        # plt.imshow(self.static_to_ref.transform(moving_mask, 'linear'))
        # plt.show()
    ...
    def optimize(self, static, moving, static_mask=None, moving_mask=None, static_grid2world=None,
        ...
        self._init_optimizer(static.astype(floating), moving.astype(floating), 
                             static_mask.astype(floating), moving_mask.astype(floating),
                             static_grid2world, moving_grid2world, prealign)
        self._optimize()
        self._end_optimizer()
