...
@cython.wraparound(False)
@cython.cdivision(True)
cdef double dice_coefficient(floating[:, :] static_mask, floating[:, :] moving_mask):
    cdef int nr = static_mask.shape[0]
    cdef int nc = static_mask.shape[1]
    cdef int i, j
    ...
    for i in range(nr):
        for j in range(nc):
            if static_mask[i, j] > 0 and moving_mask[i, j] > 0:
                intersection += 1
            if static_mask[i, j] > 0 or moving_mask[i, j] > 0:
                total += 1
...
def compute_ssd_demons_step_2d(floating[:,:] delta_field,
                               ...
                               double sigma_sq_x,
                               floating[:,:,:] out,
                               floating[:,:] static_mask,
                               floating[:,:] moving_mask):
    ...
    cdef double dice_score = dice_coefficient(static_mask, moving_mask)
    ...
    # Define weights for SSD and Dice contributions
    cdef double alpha = 1.0  # Weight for SSD
    cdef double beta = 1.0   # Weight for Dice, can be negative if Dice is a similarity measure (1 - Dice Score)

    # Calculate total energy
    cdef double total_energy = alpha * energy + beta * (1 - dice_score)

    return np.asarray(out), total_energy
