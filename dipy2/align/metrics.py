...
from dipy2.align import custom_optimisation as ssd2
...
class SimilarityMetric(metaclass=abc.ABCMeta):
    ...
    def set_static_mask(self, static_mask, static_affine, static_spacing,
                         static_direction):
        r"""Sets the static mask being compared against the moving one.

        Sets the static mask. The default behavior (of this abstract class) is
        simply to assign the reference to an attribute, but
        generalizations of the metric may need to perform other operations
        ...
        self.static_mask_affine = static_affine
        self.static_mask_spacing = static_spacing
        self.static_mask_direction = static_direction
    ...
    def set_moving_mask(self, moving_mask, moving_affine, moving_spacing,
                         moving_direction):
        r"""Sets the moving mask being compared against the static one.

        Sets the moving mask. The default behavior (of this abstract class) is
        simply to assign the reference to an attribute, but
        generalizations of the metric may need to perform other operations
        ...
        self.moving_mask_affine = moving_affine
        self.moving_mask_spacing = moving_spacing
        self.moving_mask_direction = moving_direction
    ...
    def dice_coefficient(self, static_mask, moving_mask):
...
class SSDMetric(SimilarityMetric):
    ...
    def __init__(self, dim, smooth=4, inner_iter=10, step_type='demons', alpha=0.5, beta=0.5):
    ...
    def initialize_iteration(self):
        ...
        self.gradient_moving_mask = np.empty(
            shape=self.moving_mask.shape+(self.dim,), dtype=floating)
        for i, grad in enumerate(gradient(self.moving_mask)):
            self.gradient_moving_mask[..., i] = grad

        # Convert static image's gradient field from voxel to physical space
        if self.moving_mask_spacing is not None:
            self.gradient_moving_mask /= self.moving_mask_spacing
        if self.moving_mask_direction is not None:
            self.reorient_vector_field(self.gradient_moving_mask,
                                       self.moving_mask_direction)
        ...
        self.gradient_static_mask = np.empty(
            shape=self.static_mask.shape+(self.dim,), dtype=floating)
        for i, grad in enumerate(gradient(self.static_mask)):
            self.gradient_static_mask[..., i] = grad

        # Convert static image's gradient field from voxel to physical space
        if self.static_mask_spacing is not None:
            self.gradient_static_mask /= self.static_mask_spacing
        if self.static_mask_direction is not None:
            self.reorient_vector_field(self.gradient_static_mask,
                                       self.static_mask_direction)
    ...
    def compute_demons_step(self, forward_step=True):
        ...
        """
        sigma_reg_2 = np.sum(self.static_spacing**2)/self.dim
        sigma_reg_2_mask = np.sum(self.static_mask_spacing**2)/self.dim

        if forward_step:
            gradient = self.gradient_static
            gradient_mask = self.gradient_static_mask
            delta_field = self.static_image - self.moving_image
            delta_field_mask = self.static_mask - self.moving_mask
        else:
            gradient = self.gradient_moving
            gradient_mask = self.gradient_moving_mask
            delta_field = self.moving_image - self.static_image
            delta_field_mask = self.moving_mask - self.static_mask

        if self.dim == 2:
            # step, self.energy = ssd2.compute_ssd_demons_step_2d(delta_field,
                                                            #    gradient, delta_field_mask, gradient_mask,
                                                            #    sigma_reg_2, alpha=0, beta=1,
                                                            #    out=None)
            step, energy = ssd.compute_ssd_demons_step_2d(delta_field,
                                                               gradient,
                                                               sigma_reg_2,
                                                               None)
            step_mask, energy_mask = ssd.compute_ssd_demons_step_2d(delta_field_mask,
                                                                gradient_mask,
                                                                sigma_reg_2_mask,
                                                                None)
        else:
        ...
        # Additional term for comparing segmentation masks using Dice coefficient
        dice = self.dice_coefficient(self.static_mask, self.moving_mask)
        bce = BinaryCrossentropy()
        loss = bce(self.static_mask, self.moving_mask)
        
        # if loss.numpy() < 1:
        #     alpha = dice/4
        #     beta = (1 - dice) * 4
        #     # alpha = (dice + float(loss.numpy()))/4
        #     # beta = ((1-dice) + (1 - float(loss.numpy())))
        # else:
        alpha = 0
        beta = 1

        self.energy = self.alpha * energy + self.beta * (energy_mask)
        step = (self.alpha * step + self.beta * step_mask)
